...
Usage:
  ...
  python test_runner.py --base-url http://127.0.0.1:9002   # live server
...
class Ctx:
    def __init__(self, base_url="http://127.0.0.1:9002", verbose=False):
    ...
    def need_server(self):
        if not self.server_available():
            skip("Server not running — start: uvicorn backend.main:app --port 9002")
...
# RUNNER
# =============================================================================
def run(base_url="http://127.0.0.1:9002",module_filter=None,tc_filter=None,verbose=False,report_path=None):
    ...
    if skipped>0 and not module_filter and not tc_filter:
        ...
        if server_skips: print(f"     • {server_skips} live tests — start ALVA server: {W('uvicorn backend.main:app --port 9002')}")
...
def main():
    ...
    p.add_argument("--base-url",default="http://127.0.0.1:9002")
