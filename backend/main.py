...
from backend.session_store import get_session, save_session, clear_session, update_fsm_state, increment_total_calls, get_total_calls
from backend.nlu import extract_nlu
from backend.dialogue_manager import generate_reply, feedback, noshow_dialogue, get_missing_slot_prompt, increment_clarif_count, reset_clarif_count, clarif_limit_reached, build_readback, get_silence_reply, slot_prompts
...
# ──────────────────────────────────────────────────
# TC043: Tentative appointment expiry background task
# Runs every 60 s; cancels TENTATIVE appointments
# older than 30 minutes that were never confirmed.
# ──────────────────────────────────────────────────
import asyncio as _asyncio
from datetime import timezone as _tz

async def _expire_tentative_appointments():
    """TC043: Periodically expire stale TENTATIVE appointments."""
    while True:
        await _asyncio.sleep(60)
        try:
            from backend.db import get_all_appointments, update_appointment_status
            now = datetime.utcnow()
            for appt in get_all_appointments():
                if appt.get("state") != "TENTATIVE":
                    continue
                created = appt.get("created_at") or appt.get("date_time")
                if not created:
                    continue
                ...
                if age_minutes > 30:
                    ...
                    print(f"[TC043] Expired TENTATIVE appointment id={appt['id']}")
        except Exception as _e:
            print(f"[TC043] Expiry task error: {_e}")


@app.on_event("startup")
async def _startup_tasks():
    """TC043: Launch background jobs on server start."""
    _asyncio.create_task(_expire_tentative_appointments())
    ...
    try:
        ...
        while True:
            ...
            if text.strip() == "__silence_timeout__":
                ...
                # TC016/TC054: tier 1 reprompt at count==1, tier 2 at count==2, escalate at 3+
                reply_text, should_escalate = get_silence_reply(silence_count)
                ...
                if should_escalate:
                    ...
                    await websocket.send_json({
                        ...
                        "text":          reply_text,
                        "state":         state_machine.get_state(),
                        "silence_count": silence_count,   # TC016/TC054: tier indicator
                    })
                continue
            ...
            if session.get("post_appointment_mode"):
                ...
                if step == "rebook":
                    ...
                    update_fsm_state(session_id, "TENTATIVE")
            ...
            # so it can be acted on after the primary is resolved.
            _primary   = nlu.get("intent")
            _secondary = None
            _text_l    = text.lower()
            if _primary == "reschedule" and "confirm" in _text_l:
                _secondary = "confirm"
            elif _primary == "schedule" and "cancel" in _text_l:
                _secondary = "cancel"
            if _secondary:
                session["pending_intent"] = _secondary
            # Act on a previously stored pending_intent once primary is done
            if not _secondary and session.get("pending_intent") and _primary not in (
                "reschedule", "schedule", "cancel"
            ):
                nlu["intent"] = session.pop("pending_intent")
            ...
            if nlu.get("intent") == "confirm" and not session.get("reschedule_mode"):
                ...
                if all(session["slots"].get(k) for k in required):
                    ...
                    update_fsm_state(session_id, "CONFIRMED")
            # ---------------------------
            ...
            if session.get("reschedule_mode"):
                ...
                # ── Step 3: Have date + time → save reschedule ───────────────
                    ...
                    update_fsm_state(session_id, "RESCHEDULED")
            ...
            # TC053: if all slots filled and readback not yet done, do readback
            _all_slots_ready = all(session["slots"].get(k) for k in ["service", "date", "time", "name", "email"])
            if _all_slots_ready and not session.get("readback_done") and nlu.get("intent") not in ("confirm",):
                reply = build_readback(session["slots"])
                session["readback_done"] = True
                save_session(session_id, session)
                reset_clarif_count(session)
            ...
            # TC048: if a slot is missing, ask for it directly.
            # Do NOT short-circuit on "greeting" intent — let generate_reply() handle
            # greetings naturally so ALVA says hello before asking for slots.
            _missing_prompt = get_missing_slot_prompt(session["slots"])
            _any_slot_filled = any(session["slots"].get(k) for k in ["service","date","time","name","email"])
            if _missing_prompt and nlu.get("intent") in (None, "unknown") and _any_slot_filled:
                # TC049: guard against infinite clarification loops
                _count = increment_clarif_count(session)
                save_session(session_id, session)
                if clarif_limit_reached(session):
                    reset_clarif_count(session)
                    save_session(session_id, session)
                    result = handle_escalation(
                        session=session,
                        ...
                        reason=REASON_EXPLICIT,
                        appointment_state=state_machine.get_state(),
                        extra_log_data={"trigger": "clarif_loop_limit"},
                    )
                    if result["escalated"]:
                        room = create_room(session_id, websocket, result["context_packet"])
                        session["handoff_room_id"] = room.room_id
        ...
        # TC058: clear session only when call reached a terminal state.
        # TC042: do NOT clear during mid-call reconnects so session survives.
        _terminal_states = {"CONFIRMED", "COMPLETED", "CANCELLED", "RESCHEDULED", "NO_SHOW"}
        if state_machine.get_state() in _terminal_states:
